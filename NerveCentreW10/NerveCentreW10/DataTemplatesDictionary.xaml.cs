...
namespace NerveCentreW10
{
    public partial class DataTemplatesDictionary
    {
        ...
        private void MyGrid_PointerEntered(object sender, PointerRoutedEventArgs e)
        {
            ...
            AnimationBuilder.Create().Scale(to: new Vector2(1.1f), duration: TimeSpan.FromMilliseconds(200)).StartAsync(control);

            var control2 = preElement.FindDescendant("MyDropShadow");
            AnimationBuilder.Create().Opacity(to: (0.4f), duration: TimeSpan.FromMilliseconds(500)).StartAsync(control);
        }
